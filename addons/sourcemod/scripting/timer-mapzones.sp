...
public Action:StartTouchTrigger(caller, activator)
{
	...
	else if (g_mapZones[zone][Type] == ZtLast)
	{
		...
		if(0 < mate && IsClientInGame(mate) && IsPlayerAlive(mate)) 
		{
			...
			if(SND_TELE_LAST[0] && IsSoundPrecached(SND_TELE_LAST))
			{
				if(Client_IsValid(client, true)) EmitSoundToClientAny(client, SND_TELE_LAST);
				if(Client_IsValid(mate, true)) EmitSoundToClientAny(mate, SND_TELE_LAST);
			}
		}
		else
		{
			if(Client_IsValid(client, true) && SND_TELE_LAST[0] && IsSoundPrecached(SND_TELE_LAST)) EmitSoundToClientAny(client, SND_TELE_LAST);
			Tele_Level(client, lowestcheckpoint);
		}
	}
	else if (g_mapZones[zone][Type] == ZtNext)
	{
		if(0 < mate && IsClientInGame(mate) && IsPlayerAlive(mate) && g_mapZones[g_iClientLastTrackZone[client]][Level_Id] == g_mapZones[g_iClientLastTrackZone[mate]][Level_Id])
		{
			...
			if(SND_TELE_NEXT[0] && IsSoundPrecached(SND_TELE_NEXT))
			{
				if(Client_IsValid(client, true)) EmitSoundToClientAny(client, SND_TELE_NEXT);
				if(Client_IsValid(mate, true)) EmitSoundToClientAny(mate, SND_TELE_NEXT);
			}
		}
		else
		{
			Tele_Level(client, g_mapZones[g_iClientLastTrackZone[client]][Level_Id]+1);
			if(Client_IsValid(client, true) && SND_TELE_NEXT[0] && IsSoundPrecached(SND_TELE_NEXT)) EmitSoundToClientAny(client, SND_TELE_NEXT);
		}
...
public Action:EndTouchTrigger(caller, activator)
{
	...
	else if(g_mapZones[zone][Type] == ZtStart)
	{
		...
		if(mate == 0 && CheckIllegalTeleport(client))
		{
			...
			if(IsClientInGame(client) && SND_TIMER_START[0] && IsSoundPrecached(SND_TIMER_START))
				EmitSoundToClientAny(client, SND_TIMER_START);
	...
	else if(g_mapZones[zone][Type] == ZtBonusStart)
	{
		...
		if(IsClientInGame(client) && SND_TIMER_START[0] && IsSoundPrecached(SND_TIMER_START)) EmitSoundToClientAny(client, SND_TIMER_START);
		Timer_Restart(client);
		Timer_SetTrack(client, TRACK_BONUS);
	}
	else if(g_mapZones[zone][Type] == ZtBonus2Start)
...
public PrepareSound(String: sound[MAX_FILE_LEN])
{
	...
	if (FileExists(fileSound) || FileExists(fileSound, true))
	{
...
bool:Client_Start(client)
{
	...
	if(GetClientTeam(client) != CS_TEAM_CT && GetClientTeam(client) != CS_TEAM_T)
	{
		...
		CS_SwitchTeam(client, validteam != CS_TEAM_NONE ? validteam : GetRandomInt(CS_TEAM_T, CS_TEAM_CT));
	}
